...
arg="$1"; shift
case "${arg}" in
    create) bash "${ROOT_PROJECT_DIR}/scripts/server-create.sh" $@;;
    revoke) bash "${ROOT_PROJECT_DIR}/scripts/server-revoke.sh" $@;;
    infos) bash "${ROOT_PROJECT_DIR}/scripts/server-infos.sh" $@;;
    *) err_unknown_arg "${arg}";;
esac
